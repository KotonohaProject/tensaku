...
from .src.documents import SentenceExplanationDocument, AllTensakuDocument, QuizzesDocument, NativeExplanationDocument
...
class TensakuGenerator():
    ...
    def generate(self, essay_text: str, generate_quiz=True, generate_comment=True, generate_native_example=True, generate_native_explanation=True) -> AllTensakuDocument:
        ...
        all_explanations = self._generate_sentence_explanation_documents(essay,corrected_essay, all_mistakes)
        
        
        quizzes = self._generate_quizzes(essay, corrected_essay, all_mistakes) if generate_quiz else []
        
    
        native_example = NativeGenerator().generate(essay) if generate_native_example else None
        
        if generate_native_explanation:
            assert native_example != None, "native_example must be generated if you want to generate native_explanation"
            native_explanation = NativeExplanationGenerator().generate(original=corrected_essay.paragraph, edited=native_example.paragraph)
        else:
        ...
        all_tensaku_document = AllTensakuDocument(
            ...
            native_paragraph = native_example.paragraph,
    ...
    def _generate_quizzes(self, original_essay: Essay, corrected_essay: Essay, all_mistakes: List[List[Mistake]]) -> List[QuizzesDocument]:
        generator = QuizGenerator()
        input_dictionary_list = []
        for original_sentence, corrected_sentence, sentence_mistakes in zip(original_essay.sentences, corrected_essay.sentences, all_mistakes):
    ...
    def _get_explanation_generator(self, mistake_type: MistakeType) -> Optional[Type[ExplanationGenerator]]:
    ...
    def _generate_mistakes(self, original_essay: Essay, corrected_essay: Essay) -> List[List[Type[Mistake]]]:
        all_mistakes = []
    ...
    def _generate_sentence_explanation_documents(self, original_essay: Essay, corrected_essay: Essay, all_mistakes: List[List[Type[Mistake]]]) -> List[SentenceExplanationDocument]:
        all_explanations = []
        for original_sentence, corrected_sentence, sentence_mistakes in zip(original_essay.sentences, corrected_essay.sentences, all_mistakes):
            sentence_explanations = []
            if sentence_mistakes == []: # No mistake
                all_explanations.append(
                    SentenceExplanationDocument(original_sentence, corrected_sentence, [], self.japanese, complement_comment='Perfect!')
                )
                
            else: # there is (are) mistake(s)
                for mistake in sentence_mistakes:
                    explanation_generator = self._get_explanation_generator(mistake.type)
                    
                    if explanation_generator != None:
                        explanation = explanation_generator.generate(
                            mistake.original_sentence, 
                            mistake.corrected_sentence, mistake.get_change_prompt())
                        
                        sentence_explanations.append(explanation)
                all_explanations.append(
                    SentenceExplanationDocument(original_sentence, corrected_sentence, sentence_explanations, self.japanese)
                    )
