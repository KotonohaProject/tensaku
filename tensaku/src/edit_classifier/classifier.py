...
Edited: This movie is known to everyone in Japan."""},
    ...
    def classify(self, original_sentence: str, corrected_sentence: str, print_prompt = False) -> List[Type[Mistake]]:
        
        messages = self.initial_conversation + [{"role": "user", "content": f"Original: {original_sentence}\nEdited: {corrected_sentence}"}]
        completion = create_chat(messages=messages, gpt_config = GPTConfig(model="gpt-4"))
